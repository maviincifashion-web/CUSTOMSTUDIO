// src/customizers/Kurta/components/KurtaFoldedEmbroidery.js

// Kurta folded embroidery rendering logic, separated for clarity and maintainability.

import { parseEmbroideryValuePlacement } from './KurtaFolded';
 ...
 */
export function getKurtaFoldedEmbroideryLayers(selections) {
    ...
    // Parent-child propagation rules for Kurta
    if (embByPart['base']) {
        ['sleeve', 'cuff', 'pocket', 'flap'].forEach((childPart) => {
            if (!embByPart[childPart]) {
                embByPart[childPart] = embByPart['base'].map((emb) => ({ ...emb, propagatedFrom: 'base' }));
            }
        });
    }
    if (embByPart['sleeve'] && !embByPart['cuff']) {
        embByPart['cuff'] = embByPart['sleeve'].map((emb) => ({ ...emb, propagatedFrom: 'sleeve' }));
    }
    ...
    Object.entries(embByPart).forEach(([part, embList]) => {
        embList.forEach((emb) => {
            ...
            else if (part === 'placket') zIndex = 21;
            else if (part === 'pocket') zIndex = 31;
            else if (part === 'flap') zIndex = 33;
            else if (part === 'sleeve') zIndex = 56;
            else if (part === 'cuff') zIndex = 58;
            else if (part === 'collar' || part === 'lapel') zIndex = 66;
            else if (part === 'epaulette') zIndex = 46;
            layersToRender.push({
                code: emb.code,
                zIndex: zIndex,
                type: 'embroidery',
